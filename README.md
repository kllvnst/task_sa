...
1. Регистрация устройства. При запуске мобильное приложение получает уникальный токен от ОС (iOS/Android). Приложение отправляет этот токен и UserID в Сервис регистрации устройств.
2. Сервис регистрации устройств сохраняет связку в БД устройств, указывая тип ОС. В БД мы храним связь "1 User - M устройств". Если у клиента несколько устройств - он получит уведомление на все девайсы.
